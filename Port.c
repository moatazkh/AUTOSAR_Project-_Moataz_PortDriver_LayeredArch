...
void Port_Init(const Port_ConfigType *ConfigPtr)
{
...
#endif
    {
        ...
        {
            get_Port_Pin_Num(ConfigPtr[i].portPinType);
            ...
            if (ConfigPtr[i].mode == PORT_PIN_MODE_DIO)
            {
            ...
            else if (ConfigPtr[i].mode == PORT_PIN_MODE_ADC)
            {
                CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIGITAL_ENABLE_REG_OFFSET), pinNum);    /* clear the corresponding bit in the GPIODEN register to enable digital functionality on this pin */
                CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ALT_FUNC_REG_OFFSET), pinNum);          /* Disable Alternative function for this pin by clear the corresponding bit in GPIOAFSEL register */
                *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) &= ~(0x0000000F << (pinNum * 4)); /* Clear the PMCx bits for this pin */
                SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ANALOG_MODE_SEL_REG_OFFSET), pinNum);     /* set the corresponding bit in the GPIOAMSEL register to disable analog functionality on this pin */
            }
            else
            {
                CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ANALOG_MODE_SEL_REG_OFFSET), pinNum);         /* Clear the corresponding bit in the GPIOAMSEL register to disable analog functionality on this pin */
                SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ALT_FUNC_REG_OFFSET), pinNum);                  /* enable Alternative function for this pin by clear the corresponding bit in GPIOAFSEL register */
                *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) &= ~(0x0000000F << (pinNum * 4));       /* Clear the PMCx bits for this pin */
                *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) |= (ConfigPtr[i].mode << (pinNum * 4)); /* set the PMCx bits for this pin with the desired mode */
                SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIGITAL_ENABLE_REG_OFFSET), pinNum);            /* Set the corresponding bit in the GPIODEN register to enable digital functionality on this pin */
            }
            ...
            if (ConfigPtr[i].direction == PORT_PIN_OUT)
            {
                ...
                if (ConfigPtr[i].initialValue == STD_HIGH)
                {
            ...
            else if (ConfigPtr[i].direction == PORT_PIN_IN)
            {
                ...
                if (ConfigPtr[i].resistorType == PULL_UP)
                {
                ...
                else if (ConfigPtr[i].resistorType == PULL_DOWN)
                {
...
void Port_RefreshPortDirection(void)
{
    ...
    for (uint8 i = 0; i < PORT_CONFIGURED_PORTPINS; i++)
    {
        if (Port_Configuration[i].isDirectionChangable == (STD_OFF))
        {
            get_Port_Pin_Num(Port_Configuration[i].portPinType);
            switch (portNum)
            {
            case 0:
                PortGpio_Ptr = (volatile uint32 *)GPIO_PORTA_BASE_ADDRESS; /* PORTA Base Address */
